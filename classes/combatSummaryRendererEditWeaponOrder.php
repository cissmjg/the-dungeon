...
class CombatSummaryRendererEditWeaponOrder extends CombatSummaryRenderer {
    ...
    public function init(PDO $pdo, $player_name, $character_name, &$errors) {
        ...
        if ($this->combat_summary_user_defined_weapon_order->isEmpty()) {
            foreach($this->renderers AS $renderer) {
                $renderer_index = 1;
                $this->addCombatSummaryItem($pdo, $player_name, $character_name, getMountedCombatModeDescription(COMBAT_MODE_UNMOUNTED), $renderer->getId(), $renderer_index, $errors);
                if (count($errors) > 0) {
                    die(json_encode($errors));
                }

                if ($this->is_mounted_section_needed) {
                    $this->addCombatSummaryItem($pdo, $player_name, $character_name, getMountedCombatModeDescription(COMBAT_MODE_MOUNTED), $renderer->getId(), $renderer_index, $errors);
                    if (count($errors) > 0) {
                        die(json_encode($errors));
                    }
                }
    ...
    private function addCombatSummaryItem(PDO $pdo, $player_name, $character_name, $section_name, $renderer_id, $display_order, &$errors) {
        $sql_exec = "CALL addCombatSummaryWeaponOrderItem(:playerName, :characterName, :sectionName, :rendererId, :displayOrder)";
        ...
        try {
            ...
            $errors[] = "Exception in CombatSummaryRendererEditWeaponOrder.addCombatSummaryWeaponOrderItem : " . $e->getMessage();
        }
    }
}
?>